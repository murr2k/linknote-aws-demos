  ...
  getMetricConfig(metric) {
    const configs = {
      engine_rpm: {
        ...
        max: 3000,
        thresholds: [
          { value: 2500, color: '#f39c12', label: 'High' }
        ]
      },
      engine_temp: {
      ...
      fuel_flow: {
        ...
        max: 100,
        thresholds: [
          { value: 80, color: '#f39c12', label: 'High' }
        ]
      },
      battery_soc: {
      ...
      generator_load: {
        ...
        max: 100,
        thresholds: [
          { value: 90, color: '#e74c3c', label: 'High Load' }
        ]
      },
      speed: {
        label: 'Speed (knots)',
      ...
      bilge_level: {
        label: 'Bilge Level (cm)',
        color: '#2980b9',
        backgroundColor: 'rgba(41, 128, 185, 0.1)',
        min: 0,
        max: 100,
        thresholds: [
          { value: 60, color: '#e74c3c', label: 'Critical' },
          { value: 40, color: '#f39c12', label: 'Warning' }
        ]
      },
      co2_level: {
        label: 'CO2 Level (ppm)',
        ...
        max: 2000,
        thresholds: [
          { value: 1000, color: '#e74c3c', label: 'High' },
          { value: 800, color: '#f39c12', label: 'Warning' }
        ]
      }
    };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Request initial data when a vessel is selected
  setTimeout(() => {
    if (window.dashboard && window.dashboard.selectedVessel) {
      // Request historical data for the selected vessel
      if (window.wsManager && window.wsManager.isConnected()) {
        window.wsManager.send({
          type: 'request_historical',
          vessel: window.dashboard.selectedVessel,
          metric: 'engine_rpm',
          timeRange: '1h'
        });
      }
    } else {
      // Show sample data if no vessel selected
